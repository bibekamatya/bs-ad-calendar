...
export const getDaysInMonth = (calendarType: 'BS' | 'AD', year: number, month: number): number => {
  if (calendarType === 'BS') {
    try {
      ...
      const nextMonth = month === 11 ? new NepaliDate(year + 1, 0, 1) : new NepaliDate(year, month + 1, 1)
...
export const getFirstDayOfMonth = (calendarType: 'BS' | 'AD', year: number, month: number): number => {
  if (calendarType === 'BS') {
    try {
  ...
  if (calendarType === 'BS') {
    try {
      ...
      return {
        ...
        formatted: {
          ...
          ad: new Date(bsDate.getEnglishYear(), bsDate.getEnglishMonth(), bsDate.getEnglishDate()).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
        }
      }
    } catch {
      const bsDateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`
    ...
    try {
      ...
      return {
        ...
        bs: bsDateStr,
        formatted: {
          ad: adDate.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
