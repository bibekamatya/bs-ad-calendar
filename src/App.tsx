    ...
    <div className="bg-linear-to-br from-gray-50 via-gray-50 to-gray-100 min-h-screen">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-center">
          ...
          <p className="text-gray-600 text-sm">
            Modern React calendar component supporting Bikram Sambat (BS) and Gregorian (AD) calendars
          </p>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-12">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 bg-white md:rounded-xl md:shadow-md md:p-8 md:ml-56">
            {activeTab === 'about' && <AboutTab />}
            {activeTab === 'basic' && (
              <BasicTab outputAD={outputs.ad1 || ''} outputBS={outputs.bs1 || ''} onADSelect={date => handleOutput('ad1', date)} onBSSelect={date => handleOutput('bs1', date)} copied={copied} onCopyCode={copyCode} renderOutput={renderOutput} />
            )}
            {activeTab === 'localization' && (
              <LocalizationTab outputAD={outputs.ad2 || ''} outputBS={outputs.bs2 || ''} onADSelect={date => handleOutput('ad2', date)} onBSSelect={date => handleOutput('bs2', date)} copied={copied} onCopyCode={copyCode} renderOutput={renderOutput} />
            )}
            {activeTab === 'range' && (
